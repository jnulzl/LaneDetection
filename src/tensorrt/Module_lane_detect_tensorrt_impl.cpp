...
CModule_lane_detect_tensorrt_impl::CModule_lane_detect_tensorrt_impl()
{
...
void CModule_lane_detect_tensorrt_impl::engine_init()
{
...
void CModule_lane_detect_tensorrt_impl::pre_process_cpu(const uint8_t *src, int src_height, int src_width,
                                               InputDataType inputDataType)
{
#ifdef AI_ALG_DEBUG
    std::cout << "-------------------Using pre_process_cpu-------------------" <<  std::endl;
#endif
    CModule_lane_detect_impl::pre_process(src, src_height, src_width, inputDataType);
...
void CModule_lane_detect_tensorrt_impl::pre_process_gpu(const uint8_t *src, int src_height, int src_width,
                                                   InputDataType inputDataType)
{
#ifdef AI_ALG_DEBUG
    ...
    std::cout << "Origin image copy to gpu : " << std::chrono::duration_cast<std::chrono::milliseconds>(finishTP1 - startTP).count() << " ms" << std::endl;
#endif
...
//        float y = (config_.net_inp_height - roi_new_height) / 2.0f;
    ...
    /******copy preprocessed data to net input pointer******/
...
void CModule_lane_detect_tensorrt_impl::pre_process(const uint8_t *src, int src_height, int src_width,
                                               InputDataType inputDataType)
{
...
void CModule_lane_detect_tensorrt_impl::engine_run()
{
#ifdef AI_ALG_DEBUG
    ...
    // get output
    auto mOutputDims = net_->getBindingDimensions(net_->getBindingIndex(config_.output_names[0].c_str()));
...
#ifdef AI_ALG_DEBUG
        std::cout << "Output shape : " << mOutputDims.d[idy] << std::endl;
#endif
    }
    if (data_out_.empty() || data_out_.size() < num_of_elems)
    {
        if(4 == mOutputDims.nbDims)
        {
            net_output_float_tensor_.batch = mOutputDims.d[0];
            net_output_float_tensor_.channels = mOutputDims.d[1];
            net_output_float_tensor_.height = mOutputDims.d[2];
            net_output_float_tensor_.width = mOutputDims.d[3];
        }
        else if(3 == mOutputDims.nbDims)
        {
            net_output_float_tensor_.batch = mOutputDims.d[0];
            net_output_float_tensor_.height = mOutputDims.d[1];
            net_output_float_tensor_.width = mOutputDims.d[2];
        }
        net_output_float_tensor_.format = NetFloatTensor::DimensionType::NCHW;
        data_out_.resize(num_of_elems);
        net_output_float_tensor_.data = data_out_.data();
    }
    float* output = static_cast<float*>(buffers_->getHostBuffer(config_.output_names[0]));
    std::copy(output, output + num_of_elems, net_output_float_tensor_.data);    
...
bool CModule_lane_detect_tensorrt_impl::constructNetwork(TRTUniquePtr<nvinfer1::IBuilder> &builder,
...
bool CModule_lane_detect_tensorrt_impl::isSupported(DataType dataType)
{
